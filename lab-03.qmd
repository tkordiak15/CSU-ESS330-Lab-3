...
```
#3.3, In Colorado Counties, there are a range of populations from 741 to 581,1596
```{r}
...
```
# maybe see if you can change the size of text for County
...
state_level_data <- question6_merged_data %>%
  ...
  mutate(
    ...
    rolling_mean_7day = rollmean(new_cases_lagged, k = 7, fill = NA, align = "right"))

ggplot(state_level_data, aes(x = date)) +
  geom_bar(aes(y = new_cases_lagged, fill = state), stat = "identity", alpha = 0.7, show.legend = FALSE) +
  geom_line(aes(y = rolling_mean_7day, color = state), size = 1.2) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("New York" = "blue", "Colorado" = "purple", "Alabama" = "red", "Ohio" = "green")) +
  scale_color_manual(values = c("New York" = "blue", "Colorado" = "purple", "Alabama" = "red", "Ohio" = "green")) +
  theme(
...
```
```{r}
state_per_capita_data %>%
  group_by(date, state) %>%
  summarize(count = n()) %>%
  filter(count > 1)
```
```{r}
state_per_capita_data <- state_per_capita_data %>%
  distinct(date, state, .keep_all = TRUE)
```
```{r}
state_per_capita_data <- as.Date(state_per_capita_data$date)
...
state_level_data <- question6_merged_data %>%
  ...
  filter(state %in% c("New York", "Colorado", "Alabama", "Ohio")) %>%
  arrange(state, date) %>%
  mutate(new_cases_lagged = daily_new_cases - lag(daily_new_cases))

state_population <- question6_merged_data %>%
  select(state, POPESTIMATE2021) %>%
  distinct()

state_per_capita_data <- state_level_data %>% left_join(state_population, by = "state") %>% mutate(new_cases_per_capita = new_cases_lagged / POPESTIMATE2021, rolling_mean_per_capita = rollmean(new_cases_per_capita, k = 7, fill = NA, align = "right"))
```
```{r}
ggplot(state_per_capita_data, aes(x = date, y = rolling_mean_per_capita, color = state)) +
  geom_line(size = 1.2) +
  labs(
    ...
    color = "State",
    caption = "Data Source: question6_merged_data"
  ) +
  theme_minimal() +
  scale_color_manual(values = c("New York" = "blue", "Colorado" = "purple", "Alabama" = "red", "Ohio" = "green")) +
  theme(
...
wmc_data <- question7_joined %>%
  group_by(date) %>%
  reframe(wm_LON = sum(LON * cases, na.rm = TRUE) / sum(cases, na.rm = TRUE),
...
ggplot() +
  ...
  geom_point(data = wmc_data, aes(x = wm_LON, y = wm_LAT, color = month, size = total_cases), alpha = 0.7) + 
  scale_size_continuous(range = c(2, 10)) +
  labs(title = "Weighted Mean Center of Covid-19 Cases Over Time",
       x = "Longitude",
       y = "Latitude",
       color = "Month",
       size = "Total Cases") + theme_minimal() + theme(legend.position = "right")
```
